...
export const addStrMap = (namespace, values, fallback = defaultFallback(namespace)) => {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (!isFunction(fallback)) {
      throw new TypeError('invalid fallback type. must be function');
    }
  }
...
export const StrMap = (values, fallback = null) => {
  if (!isPlainObject(values)) {
    throw new TypeError('value of translation has invalid type. must be plainObject');
  }

  if (fallback === null) {
    fallback = () => (key, params) => {
      throwHiddenError(`Invalid strMap for "[${key}]"`);
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (Object.values(values).filter((value) => !isString(value) && !isFunction(value) && !isNumber(value)).length) {
      throw new TypeError('invalid value type of translation. must be number or string.');
    }
  ...
  return memo((key, params, defaults = fallback) => {
    checkKey(key);
    const value = values.hasOwnProperty(key) ? values[key] : defaults(key, params);
    ...
    return dehydrateParams(value, params);
  }, (key, params = null) => `${key}::${params === null ? 'null' : JSON.stringify(params)}`);
};
